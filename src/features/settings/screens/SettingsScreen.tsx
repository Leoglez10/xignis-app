...
export function SettingsScreen() {
  ...
  return (
    <main className="min-h-dvh bg-[var(--color-background)] text-[var(--color-text)]" id="main-content" tabIndex={-1}>
      <div className="mx-auto max-w-2xl px-4 pb-10 pt-[calc(1.25rem+env(safe-area-inset-top))] sm:px-6">
        <header className="mb-6 flex items-center gap-3">
          ...
          <div><p className="text-sm font-bold text-[var(--color-muted)]">Tu experiencia</p><h2 className="text-3xl font-black">Configuración</h2></div>
        </header>
        <div className="space-y-4">
          ...
          <Card className="p-5"><SettingHeading icon={<PackageOpen />} title="Versión" /><dl className="grid grid-cols-2 gap-3 text-sm"><div><dt className="text-[var(--color-muted)]">Aplicación</dt><dd className="mt-1 font-black">v{version}</dd></div><div><dt className="text-[var(--color-muted)]">Build</dt><dd className="mt-1 font-black">{new Date(build).toLocaleDateString("es-MX")}</dd></div></dl><Button className="mt-5 w-full" variant="secondary" onClick={() => void checkUpdate()}>Buscar actualizaciones</Button></Card>
        </div>
      </div>
    </main>
  );
...
function SettingHeading({ icon, title }: { icon: ReactNode; title: string }) { return <div className="mb-4 flex items-center gap-2"><span aria-hidden="true" className="[&>svg]:size-5">{icon}</span><h3 className="text-lg font-black">{title}</h3></div>; }
