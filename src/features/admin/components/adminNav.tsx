import { BarChart3, LayoutDashboard, Settings, UserCircle, Users, type LucideIcon } from "lucide-react";
...
export function AdminBottomNav() {
  ...
  return (
    <nav
      ...
      className="fixed inset-x-0 bottom-0 z-30 flex items-stretch justify-around border-t border-slate-200 bg-white/95 pb-[env(safe-area-inset-bottom)] backdrop-blur md:hidden"
    >
      {adminNavItems.map((item) => {
        const Icon = item.icon;
        const active = isActive(pathname, item.to);
        return (
          <button
            aria-current={active ? "page" : undefined}
            className={`press flex flex-1 flex-col items-center gap-1 py-2.5 text-[11px] font-bold ${
...
/** Layout RH: sidebar (desktop) + barra inferior (mobile) + contenido. */
export function AdminShell({ children }: { children: React.ReactNode }) {
  return (
    <main className="min-h-dvh bg-slate-50 text-[var(--color-text)]" id="main-content" tabIndex={-1}>
      <div className="grid min-h-dvh md:grid-cols-[236px_1fr]">
        <AdminSidebar />
        <div className="min-w-0 pb-24 pt-[env(safe-area-inset-top)] md:pb-0 md:pt-0">{children}</div>
      </div>
      <AdminBottomNav />
    </main>
  );
